...
if osver == "Darwin":
	...
	if len(sys.argv) < 2:
		...
		if tmpdir is None:
			...
			shutil.copyfile(thisfile, f"{tmpdir}/pset7.py")
		...
		os.execlp(sys.executable, sys.executable, f"{tmpdir}/pset7.py", device)
		prbad("WTF???")
	...
	def activate_venv():
		...
		os.environ["VIRTUAL_ENV_PROMPT"] = "(pset7)"
...
clearScreen()
print(f"PSET7 {VERSION} SETUP by Parzival")
...
def sanity():
	...
	if fileSanity > 0:
		...
		prinfo("Please re-extract the PSET7 zip file, overwriting any existing files when prompted.")
		exitOnEnter()
...
def injection():
	...
	prgood("PSET7 successfully injected!")

def remove():
	global fs, realId1Path, id0, id1
	prinfo("Removing PSET7...")
	...
	id1 = id1[:32]
	realId1Path = id0 + "/" + id1
	prgood("Successfully removed PSET7!")
...
for root, dirs, files in fs.walk("Nintendo 3DS/", topdown=True):
	...
	for name in dirs:
		# If the name doesn't contain sdmc (Ignores PSET7 exploit folder)
	...
	for name in dirs: # Run the check for existing install after figuring out the structure
		# Check if we have an PSET7 Hacked id1 folder
		if "sdmc" in name and len(name) == 32:
		# If the PSET7 folder doesn't match the proper haxid1 for the selected console version
			if hackedId1 != name:
				prbad("Error 03: Don't change console model/version in the middle of PSET7!")
